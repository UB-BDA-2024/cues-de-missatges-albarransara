...
def test_create_sensor_velocitat():
    response = client.post("/sensors", json={"name": "Sensor Velocitat 1", "latitude": 1.0, "longitude": 1.0, "type": "Velocitat", "mac_address": "00:00:00:00:00:01", "manufacturer": "Dummy", "model":"Dummy Vel", "serie_number": "0000 0000 0000 0000", "firmware_version": "1.0","description": "Sensor de velocitat model Dummy Temp del fabricant Dummy"})
    assert response.status_code == 200
    assert response.status_code == 200
    json = response.json()
    assert json["id"] == 2
...
def test_post_sensor_data_not_exists():
    response = client.post("/sensors/323/data", json={"temperature": 1.0, "humidity": 1.0, "battery_level": 1.0, "last_seen": "2020-01-01T00:00:00.000Z"})
    assert response.status_code == 404
    assert "Sensor not found" in response.text

def test_get_sensor_data_not_exists():
    response = client.get("/sensors/2423/data")
    assert response.status_code == 404
    assert "Sensor not found" in response.text
